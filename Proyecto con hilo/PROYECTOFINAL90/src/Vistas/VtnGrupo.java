...
public class VtnGrupo extends javax.swing.JFrame {
    ...
    public boolean validaE(String s)//valida que no haya espacios al principio y al final de la palabra
    {
        ...
        char c = s.charAt(0);
        String pl = Character.toString(c);
    ...
    private void jBAgregarGActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBAgregarGActionPerformed
        ...
        if (s == null) //Evita el NPE al salir del showInputDialog o presionar cancelar
        {
        ...
        } else if (s.length() == 0) {//AYUDA A QUE NO SE VALIDE UN CAMPO VACIO
            ...
            if (validaG(r, s) == true) {
                ...
                if (s.length() > 30) {
                    ...
                    Nodo nom = new Nodo(null, s.trim());//se pasa el nodo con .trim para eliminar ESPACIOS
                    //ESTO AYUDA A QUE NO SE VALIDEN NOMBRES IGUALES SI LE DAMOS ESPACIO AL FINAL

                    String[] etqs = new String[1];//arreglo de etiquetas,esta en 1 lo cual significa que esta en el nivel 0

                    etqs[0] = s;

                    r = Multilistas.inserta(r, nom, 0, etqs);//regresa la r, con el dato ya insertado, y la guarda en la variable estatica
                    //de grupos

                    Propiedades p = new Propiedades(r, arr);//se ocupa para guardar en el archivo, guarda la multilista y la tabla hash

                    try {
                        Archivos.guardar(p, this);//guarda en el archivo
                    } catch (FileNotFoundException ex) {
                        System.out.println("No se encontro el archivo");
                    }

                    if (r != null) {//si la lista no esta vacia entonces crea los botones
                        Nodo aux = r;//usa una variable llamada aux

                        JPGrupos.removeAll();

                        while (aux != null) {//mientras aux sea diferente de nulo, osea que no llegue al fin de la lista
                            //entonces crea los botones tomando su etiqueta
                            ...
                            boton.addActionListener(new ActionListener()//pone una accion al boton
                            {
                                ...
                                {//CUANDO PRESIONAS UN BOTON ESTE PASA EL NOMBRE DEL GRUPO A LA SIGUIENTE VENTANA QUE ES LA DE CONTACTO
                                    //ESTO AYUDA EN LA SIGUIENTE VENTANA A SABER QUE GRUPO ELEGIMOS Y EVITA QUE SE ABRA UN GRUPO DIFERENTE
                                    //, YA QUE AL INICIO BUSCA EL GRUPO QUE //LE PASAMOS AQUI, Y BAJA UN NIVEL EN DONDE SE ENCUENTRAN LOS 
    ...
    private void jBEliminarGActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBEliminarGActionPerformed
        if (r != null) {
            ...
            } else if (s.length() == 0) {//VALIDA QUE NO SEA CAMPO VACIO
                Mensaje.error(this, "Campo vacio, debe escribir un nombre");
            } else {
                if (validaG(r, s) == false)//VALIDA QUE EL GRUPO EXISTA EN LA LISTA
                {
                    ...
                    if (cjb.ci.Mensaje.pregunta(this, "Eliminar grupo\nSe eliminaran los contactos "
                        ...
                        c = true;
                    }//MENSAJE PARA VALIDAR QUE EL USUARIO NO ELIMINE POR ERROR UN DATO, PREGUNTA

                    if (c == true)//SI EL USUARIO PRESIONA ACEPTAR
                    {
                        Nodo rb = Multilistas.busca(r, s.trim());//SE BUSCA EL GRUPO EN LA MULTILISTA
                        //ESTO SE USA PARA ELIMINAR LOS NOMBRES DE ESTE GRUPO QUE SE ENCUENTRAN EN EL ARBOL

                        rb = rb.getAbj();//BAJA HASTA LOS CONTACTOS DE ESTE GRUPO
                        ...
                        if (rb != null) {
                            ...
                            while (aux != null) {
                                ...
                                arr.elimina(aux.getEtq());//ELIMINA DEL ARBOL BINARIO LOS CONTACTOS QUE PERTENECEN A ESTE GRUPO

                                aux = aux.getSig();
                            }
                        }
//EL ELIMINA DE LA MULTILISTA SE DA DESPUES PORQUE PRIMERO ES NECESARIO ELIMINAR DEL ARBOL, PORQUE ES NECESARIO SABER A QUE GRUPO PERTENECEN LOS NOMBRES
                        r = Multilistas.elimina(r, 0, etqs);//SE ELIMINA DE LA MULTILISTA, Y SE PASA EL NUEVO VALOR A R

                        Propiedades p = new Propiedades(r, arr);//SE HACE UN OBJETO DE PROPIEDADES PARA GUARDAR EN EL ARCHIVO
                        ...
                        if (r != null) {//PARA ELIMINAR LOS BOTONES, SI R ES DIFERENTE DE NULO, QUIERE DECIR QUE LA MULTILISTA TIENE VALORES
                            System.out.println("Estoy entrando a esta parte del codigo");
                            Component componentes[] = JPGrupos.getComponents();//SE CREA UN ARREGLO DE COMPONENTES QUE SE ENCUENTRAN EN EL JPANEL
//ESTO SE USA PARA QUE SEA POSIBLE ACCEDER A LOS INDICES Y NOMBRES DE LOS BOTONES EN EL JPANEL
//Y QUE SE PUEDA ELIMINAR MAS FACILMENTE EL BOTON

                            for (int i = 0; i < componentes.length; i++) {//SE HACE UN CICLO FOR
                                //SE COMPARA SI ETQS EN LA POSICION 0 ES IGUAL A COMPONENTES EN EL INDICE I
                                if (etqs[0].trim().equalsIgnoreCase(((JButton) componentes[i]).getText().trim())) {
                                    JPGrupos.remove(i);//SI ES IGUAL ELIMINA EL COMPONENTE DEL JPANEL DE ACUERDO A SU INDICE
                                }
                            }
                        } else {//SI LA LISTA YA NO TIENE MAS ELEMENTOS ENTONCES ELIMINA TODO LO QUE TIENE EL PANEL
                            JPGrupos.removeAll();
                        }
                        //////////////////
                    }
    ...
    private void jBBuscaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBBuscaActionPerformed
        ...
        if (s == null) //Evita el NPE al salir del showInputDialog o presionar cancelar
        {
        ...
        } else if (validaE(s) == false) {//VALIDA QUE EL PRIMER CARACTER DE UN CONTACTO QUE SE QUIERA BUSCAR NO SEA ESPACIO
            ...
            if (valida(s) == false) {
                ...
                int pos = s.toUpperCase().codePointAt(0) - 65;//BUSCA LA POSICION EN LA TABLA HASH EN LA CUAL VA A BUSCAR
                //ESTO LO HACE TOMANDO LA PRIMERA LETRA DEL NOMBRE, 

                if (arr.getArr()[pos] != null) {////si la posicion en el arbol es diferente de nulo
